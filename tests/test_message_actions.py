...
async def test_messages_visible_after_seed(
    client: AsyncClient,
    test_conv_title: str,
) -> None:
    cid, _, _ = await _seed_conversation(client, test_conv_title)
    resp = await client.get(f"/api/conversations/{cid}/messages")
    assert resp.status_code == 200
    assert len(resp.json()) == 2


async def test_delete_user_cascades_following(
    client: AsyncClient,
    test_conv_title: str,
) -> None:
    cid, user_id, _assistant_id = await _seed_conversation(client, test_conv_title)
...
async def test_delete_assistant_only(client: AsyncClient, test_conv_title: str) -> None:
    cid, user_id, assistant_id = await _seed_conversation(client, test_conv_title)
...
async def test_patch_assistant_without_cascade(
    client: AsyncClient,
    test_conv_title: str,
) -> None:
    cid, _user_id, assistant_id = await _seed_conversation(client, test_conv_title)
