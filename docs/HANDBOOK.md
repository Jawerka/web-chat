# web-chat — архитектура, принципы и план разработки (HANDBOOK)
...
> **Статус (2026-06-28):** этапы **1–11** и стабилизация **P0–P2 (пилот)** выполнены. Добавлены **SD gallery bridge** (`/api/sd-bridge/import`), Chrome **booru-web-chat**, расширения в [`extensions/`](extensions/). Автотесты: `pytest -q` (см. `tests/`). Production: **PostgreSQL**, login/password, multi-user, RAG, корзина, trusted internal. Журнал — [§журнал](#журнал-прогресса); открытые задачи — [§22](#22-планируемые-действия).
...
0. [Видение и цели](#0-видение-и-цели) — в т.ч. [§0.5 Модель эксплуатации](#05-модель-эксплуатации-и-приоритеты-разработки)  
...
### 0.5. Модель эксплуатации и приоритеты разработки

Это **базовое допущение проекта**. Любая новая фича, рефакторинг и «усиление безопасности» должны сверяться с этим разделом **до** реализации.

#### Для кого и где работает

- Приложение **не предназначено для публичного интернета** и массового доступа. Напротив: это **личный инструмент** в домашней или офисной LAN (при необходимости — VPN), которым пользуется **один человек** (редко — несколько доверенных учёток в той же сети).
- Здесь **нет** хранения особо чувствительных данных (банки, персональные документы с юридической значимостью, корпоративные секреты). Контент — чат, картинки, черновики промптов, бытовые PDF для анализа.

#### Что из этого следует для разработки

| Приоритет | Смысл |
|-----------|--------|
| **Гладкость UX** | Предсказуемый чат, resume после F5, понятные статусы генерации, корзина, быстрый composer, минимум трения при img2img/RAG/вложениях. |
| **Надёжность своего стенда** | LLM/SD доступны, `PUBLIC_BASE_URL` верный, бэкап БД, `/health`, логи — без «enterprise» избыточности. |
| **Простота доступа** | Не плодить лишние **припоны** к входу: лишние пароли, жёсткий rate limit «от злоумышленника», prompt-injection hardening, Redis-сессии, вынос MCP в отдельный контур — **не цели** по умолчанию. |
| **Базовая гигиена** | `safe_filename`, доверенные URL медиа, не светить `.env` — достаточно для LAN; опционально auth/proxy, если сеть шире одного пользователя. |

#### Что **не** гоним в продукт без явной просьбы

- Модель угроз «публичный SaaS» (OWASP-чеклисты, агрессивная санитизация имён в промптах, DDoS rate limit).
- Горизонтальное масштабирование, message broker, кэш контекста LLM в Redis.
- Усложнение UI ради «корпоративного» вида в ущерб скорости итераций.

#### Auth и SECURITY в коде

В репозитории есть **P0–P2** механизмы (login, API key, rate limit, trusted internal для LLM/SD) — они полезны для **гибкости** и тестов, но для **одного оператора в LAN** допустимо:

- `AUTH_ENABLED=false` на dev-стенде;
- пустые `API_ACCESS_KEY` / `TRUSTED_WS_ORIGINS` в доверенной подсети;
- смягчённые `RATE_LIMIT_*` в `.env`.

Включать auth и proxy имеет смысл, если хост доступен **не только вам** (гостевая Wi‑Fi, несколько пользователей). Подробности — [SECURITY.md](SECURITY.md), [deploy/AUTH.md](../deploy/AUTH.md).

> **Правило для ревью:** если изменение заметно ухудшает UX ради гипотетической атаки из интернета — отклонять или делать за явным флагом. Открытые задачи — [§22](#22-планируемые-действия).
...
MediaAsset
  ...
  llm_data        bytes nullable  # кэш JPEG для GET …/llm (vision)
...
├── tests/          pytest (auth, RAG, WS, sd-bridge, from-image, …)
├── extensions/
│   booru-web-chat/           Chrome MV3 → from-image
│   sd-webui-web-chat-bridge/ SD WebUI → img2img queue
├── deploy/
...
├── .env.example, README.md, docs/HANDBOOK.md, docs/Sys-prompt.md, docs/SECURITY.md, docs/RUNBOOK.md
...
> Философия «один пользователь, LAN, UX важнее паранойи» — [§0.5](#05-модель-эксплуатации-и-приоритеты-разработки).

**Базовый контур (достаточно для домашнего стенда):**

- Сервис в LAN; **не** выставлять порт в открытый интернет без reverse proxy.
- `safe_filename()` + `Path.resolve().is_relative_to()` для путей; доверенные URL медиа (SSRF для tools).
- Санитизация HTML на клиенте (`sanitizeHtml`).
- Секреты в `.env`, не в git.
- **Trusted internal** — LLM/SD без cookie к `/media/asset/*` ([deploy/AUTH.md](deploy/AUTH.md)).

**Опционально** (если в сети не только вы): `AUTH_ENABLED`, `API_ACCESS_KEY`, nginx + HTTPS, `TRUSTED_WS_ORIGINS`, rate limit — [SECURITY.md](SECURITY.md).

**WireGuard (позже):** UI за VPN; `PUBLIC_BASE_URL` должен быть достижим из браузера через туннель.
...
### 2.9. UX важнее «security theater»

При конфликте требований побеждает [§0.5](#05-модель-эксплуатации-и-приоритеты-разработки): не добавлять шаги входа, модальные предупреждения и лимиты «на всякий случай», если они мешают ежедневному сценарию (чат → картинка → img2img → F5). Защита от реальных багов (path traversal, битый URL, зависший turn) — да; защита от гипотетического хакера из интернета — только за флагом или в [SECURITY.md](SECURITY.md) для расширенного контура.
...
> **Архив.** Этапы 1–11 и чеклисты ниже — исторический план первичной разработки (всё выполнено). Открытые задачи — [§22](#22-планируемые-действия); платформа P0–P2 — [§21](#21-стабилизация-и-платформа-v2-2026-05-23). Новый код сверять с [§0.5](#05-модель-эксплуатации-и-приоритеты-разработки), а не с чекбоксами этапов.
...
> Для **личного LAN** достаточно пунктов про LLM/SD/`PUBLIC_BASE_URL`/бэкап. Auth, API key и nginx — [§0.5](#05-модель-эксплуатации-и-приоритеты-разработки), если хост не только ваш.
...
- [ ] (Если сеть не только ваша) `AUTH_ENABLED=true`, `AUTH_SECRET` (≥32), смена пароля `admin` — [deploy/AUTH.md](deploy/AUTH.md).
- [ ] (Если сеть не только ваша) `API_ACCESS_KEY` и/или Basic Auth на proxy — [SECURITY.md](SECURITY.md).
- [ ] (Если сеть не только ваша) `TRUSTED_WS_ORIGINS` при доступе не с localhost.
...
| Контекст LLM | `build_conversation_llm_context`: warning в лог, если сборка истории >300 ms |
...
- ~~`reasoning_delta` в UI~~ — реализовано (2026-05-24): WS + `content_json.reasoning`; видно только если модель отдаёт reasoning в stream API.
...
- [x] `pytest -q` → **289** passed (2026-05-24).
...
| docs sync | [x] | 2026-05-23 | HANDBOOK §21–22, BACKLOG, README/SECURITY/DEPLOY |
| docs phase 0 | [x] | 2026-05-24 | HANDBOOK/BACKLOG rename, pytest gallery fix, README trash/health |
| trash | [x] | 2026-05-24 | Корзина бесед, retention, API trash/restore |
| trusted internal | [x] | 2026-05-24 | LLM/SD без cookie, sync URL из UI |
| product model §0.5 | [x] | 2026-05-24 | Один оператор, LAN, UX > security theater; SECURITY переписан |
| llm_data cache | [x] | 2026-05-24 | `MediaAsset.llm_data`, миграция, повторный `/llm` без PIL |
| chat-labels.js | [x] | 2026-05-24 | Подписи статусов вынесены из chat.js |
| docs/RUNBOOK | [x] | 2026-05-24 | [docs/RUNBOOK.md](docs/RUNBOOK.md) |
| RAG sources UI | [x] | 2026-05-24 | Блок источников под ответом ассистента |
| resume images | [x] | 2026-05-24 | F5/resume без дубля картинок в сетке |
| llm context timing | [x] | 2026-05-24 | Warning >300 ms в `llm_context.py` |
| rag_sources persist | [x] | 2026-05-24 | `content_json.rag_sources` при ходе с RAG; UI из БД после F5 |
...
> Стабилизация P0–P2 зафиксирована в этом разделе. Открытые задачи — [§22](#22-планируемые-действия).
...
**Автотесты:** `pytest -q` (см. [§14.4](#144-парадигма-pytest-обязательно-для-новых-тестов)).
...
### P1 — эксплуатация (по желанию; для одного пользователя в LAN не обязательно)
...
> Детальный план — [§22](#22-планируемые-действия). Инциденты — [docs/RUNBOOK.md](docs/RUNBOOK.md).
